...
local Window = Rayfield:CreateWindow({
    Name = "Lumeware.wth (Owner)",
    Icon = 0,
    LoadingTitle = "Lumeware.wth (Owner)",
    LoadingSubtitle = "By Tekzo",
    Theme = "Amethyst",
...
if L.vhsows then
	local debugtab = Window:CreateTab("General", 7743876054)

		debugtab:CreateSection("Main")

			local debugl = debugtab:CreateLabel("Hello, welcome to Lumeware.wth!")
...
local usertab = Window:CreateTab("User stuff", 7743876054)
...
local grabtab = Window:CreateTab("Line mods", 7733954884)
...
local blobtab = Window:CreateTab("Kick players", 7733916988)
...
local scripttab = Window:CreateTab("Random", 7733920644)
...
local ColorSpeed = 0.1  
local RainbowDuration = 0.1 
local FlashCount = 1  
local FlashSpeed = 0.15  
local TextSize = 18  
Modules.RainbowColors = {
    Color3.fromRGB(255, 0, 0),  
    Color3.fromRGB(255, 255, 0), 
    Color3.fromRGB(0, 255, 0),   
    Color3.fromRGB(0, 255, 255), 
...
local FlashColor = Color3.fromRGB(0, 255, 0) 

Modules.DisplayMessage = function(textRainbow, textFlash)
    local colorIndex = 1
    local startTime = os.clock()
    while os.clock() - startTime < RainbowDuration do
        local currentColor = Modules.RainbowColors[colorIndex]
        Modules.Services.StarterGui:SetCore("ChatMakeSystemMessage", {
            Text = textRainbow,
            Color = currentColor,
            Font = Enum.Font.SourceSansBold,
            TextSize = TextSize
        ...
        colorIndex = (colorIndex % #Modules.RainbowColors) + 1
        task.wait(ColorSpeed)
    end
    for i = 1, FlashCount do
        task.wait(FlashSpeed)
        local flashText = (i % 2 == 0) and "" or textFlash
        Modules.Services.StarterGui:SetCore("ChatMakeSystemMessage", {
            Text = flashText,
            Color = FlashColor,
            Font = Enum.Font.SourceSansBold,
...
Modules.DisplayMessage("Made by Tekzo", "#1 Header")
local Modules = {}
Modules.Services = {
    RunService = game:GetService("RunService"),
    StarterGui = game:GetService("StarterGui")
...
Modules.DisplayMessage = function(textRainbow, textFlash, symbol)
    local progress = ""
    local colorIndex = 1
    ...
    while true do
        ...
        while os.clock() - startTime < RainbowDuration do
            ...
            Modules.Services.StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = string.format("[%s] %s", textRainbow, progress),
                Color = currentColor,
                Font = Enum.Font.SourceSansBold,
                TextSize = TextSize
            })
        ...
        for i = 1, FlashCount do
            task.wait(FlashSpeed)
            local flashText = (i % 2 == 0) and "" or string.format("[%s]", textFlash)
            Modules.Services.StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = flashText,
                Color = FlashColor,
                Font = Enum.Font.SourceSansBold,
...
Modules.DisplayMessage("Made by Tekzo ", "#Header 2", "Enjoy!")